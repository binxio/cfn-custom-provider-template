...
test: venv
	for TEMPLATE in ./cloudformation/*; do aws cloudformation validate-template --template-body file://$$TEMPLATE > /dev/null || exit 1 ; done
	. ./venv/bin/activate && \
...
check_prefix:
	@if [ -n "$(S3_BUCKET_PREFIX)" ] ; then echo > /dev/null; else echo ERROR: please set S3_BUCKET_PREFIX in the Makefile. >&2 ; exit 1; fi
